...
class FundQuantTradeEnv_V1(BaseTradeEnv):
    ...
    def _get_acct_pfo_shares(self):
        ...
        if update_holdings:
            ...
            # pfo_shares: 现有持仓的买入成本
            pfo_shares = sum([
                float(s['shares']) for holding in update_holdings.values()
                for s in holding
                if s['hold'] > 0 and s['soldout'] == '0'
                ])
            # pfo_asset: 现有持仓的市值
            pfo_asset = sum([
                float(s['hold']) * float(s['sell_price']) * (1 + self.live_markup)
    ...
    def _get_acct_asset(self):
        ...
        logging.warning(f'current plan id: {self.plan_id} ----> pfo_asset: {pfo_asset}, cash_asset: {cash_asset}, total asset: {total_asset}')
        return total_asset
    ...
    def _cal_max_selling_amount_with_min_yield(self, fund_code, min_yield=0.01):
        ...
        for i, h in enumerate(sort_holdings):
            ...
            logging.warning(f'''
                第 {i+1} 笔测试卖出收益 ----------->
                买入日期: {buy_date} 赎回份额: {sell_amount}
                持仓收益率: {h['yield']} 预测涨跌幅: {self.live_markup:.4f} 赎回费率: {redeem_rate}
                动态止盈收益率: {dyn_min_yield} 该笔赎回综合收益率: {selling_yield}
                ''')

            # 如果该份额卖出的收益率比动态止盈收益率低，则跳过不卖
            if selling_yield < dyn_min_yield:
                logging.warning(f'------------> 没有找到达到预期目标 {dyn_min_yield} 收益的持仓')
                break
            ...
            logging.warning(f'-----------> {fund_code} 累计前 {i+1} 笔已盈利持仓的综合赎回收益率: {total_selling_yield:.4f}')
    ...
    def _get_pfo_soldout_yield(self):
        ...
        fundcode_assets = {}
        if update_holdings:
            for fundcode, holdings in update_holdings.items():
                fundcode_assets.setdefault(fundcode, 0)
                for h in holdings:
                    if h['soldout'] == '0':
                        fundcode_assets[fundcode] += float(h['hold'])

        total_redem_fee = 0
        total_hold_shares = 0
        for fundcode, hold_amount in fundcode_assets.items():
            fifo_redem_rate = self._cal_fifo_redeem_rate(fundcode, hold_amount, mode='Backtest')
            total_redem_fee += hold_amount * fifo_redem_rate
            total_hold_shares += hold_amount
        total_redem_rate = round(total_redem_fee / total_hold_shares, 4) if total_hold_shares != 0 else 0
        # TODO: 暂时将清仓费率统一设置为 0.75%
        # total_redem_rate = 0.75 / 100

        pfo_yield = pfo_asset / pfo_shares - 1 if pfo_shares > 0 else 0
        soldout_return = pfo_yield - total_redem_rate
        logging.warning(f'----------> 当前清仓，整体定投的收益率: {soldout_return:.4f}')
        return soldout_return
    ...
    def buying_signal(self, index):
        ...
        # 主要风险：
            # 1. 当预测跌1天，指数连续下跌3～5天以上，导致模型建议在连跌的时候持续买入。因此还要做一个辅助模
        chaodi_signal = any([
            all([
                ...
                # 重要‼️ 保证 _mark_point 在预测误差范围内
                _mark_point >= lb_point and _mark_point <= ub_point,
                ]),
            # 人为设定最大回撤天数，必须加仓。可以使用绝对点位, 待使用模型分析, 可以统计中位数
            # 如果超过该阈值，则每天都加仓
            _mark_point <= self.days_slope,
            ])

        # 如果超跌，需要抄底
        # TODO: -3.5 阈值也可以数据分析
        if _mark_point <= self.days_slope or self.live_markup <= -3.5:
            self.buy_times = 1.5
        ...
        if self.mode in ['live']:
            logging.warning(f'''
                --------> 
                当前涨跌势: {_mark_point}, 预测走势: {_pred_points}, 测试损失: {test_loss}
                抄底范围 lb_point: {lb_point:.1f} ~ ub_point: {ub_point:.1f}，温度：{(_pred_points * (1 - self.temperature)):.1f}
                超跌交易倍数: {self.buy_times}
                是否抄底信号: {chaodi_signal}
